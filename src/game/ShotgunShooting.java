...
public class ShotgunShooting {
	
	public void shoot(int x, int y, GameMap map, String direction) {
		...
		for (int i = 0; i < highDamage.size(); i ++) {
			...
			if (map.at(highDamage.get(i).x(), highDamage.get(i).y()).containsAnActor() == true) {
				if (random1 <= 0.75) {
					...
					target.hurt(70);
					System.out.println(target.toString() + " has been shot by a shotgun for 70 damage !");
				}
			}
		}
		for (int i = 0; i < mediumDamage.size(); i ++){
			...
			if (map.at(mediumDamage.get(i).x(), mediumDamage.get(i).y()).containsAnActor() == true) {
				if (random2 <= 0.75) {
					...
					target.hurt(40);
					System.out.println(target.toString() + " has been shot by a shotgun for 40 damage !");
				}
			}
		...
		for (int i = 0; i < lowDamage.size(); i ++) {
			...
			if (map.at(lowDamage.get(i).x(), lowDamage.get(i).y()).containsAnActor() == true) {
				if (random3 <= 0.75) {
					...
					target.hurt(20);
					System.out.println(target.toString() + " has been shot by a shotgun for 20 damage !");
					}
				}
			}
		}
